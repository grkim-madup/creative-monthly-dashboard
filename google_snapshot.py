...
- 자동 고정은 하지 않는다(2026-08-20 결정 — 자동으로 몰래 얼려지는 것보다, 사람이 그 달
  보고서를 마감하는 시점에 직접 눌러서 고정하는 편이 명확하다). 스냅샷이 없는 달은 그냥
  계속 라이브 값을 보여준다.
- 사용자가 "지금 시점으로 고정" 버튼을 누르면 그 시점 값을 얼린다(최신 달이든 지난 달이든
  상관없다). 이미 고정된 달을 다시 누르면 그 시점 값으로 재고정된다.
